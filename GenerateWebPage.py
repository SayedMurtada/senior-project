...
#this function write the php part before the head of the page
def beforeHTML(userID,wantedtables, filename, dbtype="sqlite"):
    tableslength = len(wantedtables)
    f = open(filename + ".php", "a+")
    filename = os.path.basename(filename)
    #here the function write the session part in the file.
    f.write("<?php \n")  # done
    f.write("session_start();\n")
    f.write("if(empty($_SESSION['user'])){\n")
    f.write("   $_SESSION['user']='" + str(userID) + "';\n")
    f.write("}\n")
    # here the function decide on the check for connection way depending on the database type
    if (dbtype != "sqlite"):
        f.write(
            "if (isset($_SESSION['" + filename + "HostDB']) && isset($_SESSION['" + filename + "UserDB']) && isset($_SESSION['" + filename + "DatabaseDB'])) {\n")
    else:
        f.write("if (isset($_SESSION['" + filename + "PathDB'])) {\n")
    if (dbtype != "sqlite"):
        f.write("    try{\n")
    ...
    f.write("if (isset($_REQUEST['LookandFeelButton'])) {\n")
    f.write('    $_SESSION["PageName"] = "' + filename + '";\n')
    f.write("    echo'<meta http-equiv=" + '"refresh"' + 'content="0; URL=SelectLookAndFeelPage.php" />' + "';\n")
    f.write("}\n")
    # if (isset($_REQUEST['PageName'])) {
    # $_SESSION["PageName"] = "MySQL_Project";
    # echo '<meta http-equiv="refresh" content="0; URL=SelectLookAndFeelPage.php" />';
    # }
    #here the functions checks some session variables and assign them some value
    # the if statements safe the current page for later use
    ...
    f.write("}\n")
    # the if statements safe the current page for each page for later use
    for i in range(tableslength):
        f.write("if(isset($_REQUEST['" + wantedtables[i] + "'])) {\n")
        f.write("    $_SESSION['" + filename + "CurrentPage'] = $_GET['" + wantedtables[i] + "'];\n")
        f.write('}\n')
    f.write("?>\n")
    f.close()

# as the name of the function it writes the head part of the page.
def Head(userID, filename):
    f = open(filename + ".php", "a+")
    filename = os.path.basename(filename)
    f.write("    <head>\n")
    f.write("        <?php\n")
    #this if statement writes the title of th page
    f.write("        if(!empty($_SESSION['" + filename + "Title'])){\n")
    ...
    f.write("        }\n")
    f.write("        ?>\n")
    #here the main settings of the page are being written from languages,sizes, to scripts and stylesheets
...
#this function write the functionalities of thenav bar of the page and the logo
def NavBar(wantedtables, filename, dbtype="sqlite"):
    ...
    f.write("            ?>\n")
    f.write("            <hr>\n")
    #here the button of the connect request
    f.write(
    ...
    f.write("                    <?php\n")
    #this if statement decide on the database type and write the right connection code for that database
    if (dbtype != "sqlite"):
        f.write(
        ...
        f.write('                        echo"<button class=' + "'btn btn-outline-success my-2 my-sm-3'" + " name='"
                + "LookandFeelButton'" + " value='" + filename + "' type='" + "submit'" + '>Look and feel</button>";\n')
        f.write('                        echo"</li>";\n')
        f.write('                        echo"</form>";\n')
        # <form>
    ...
    # this if statement check the database type and write the code for checking the connection information
    # if they are empty or not. if their not create a connection for query later
    if (dbtype != "sqlite"):
        f.write(
...
# this function writes the button for navigation from one table(page) to another
...
# this function write the code for search queries and the tables and the functionalities that works on
...
def SearchAndTable(wantedtables, wantedcolumns, filename, dbtype="sqlite"):
    ...
    f.write("                <?php\n")
    # first check the connection if it is not empty contenue printing the search and table components
    ...
    f.write('>";\n')
    che = 0
    #print if statement for each table(Page) to check what data should be used for each search function and table functions.
    for i in range(tableslength):
        ...
        for j in range(columnslength):
            if (wantedcolumns[j][0] == i):
                ...
                if (int(wantedcolumns[j][2]) == 1 and che == 0):
    ...
    # here the if statement checks if there is a search request.
    # it is composed of two parts.
    # the first part is if the input is only spaces which is empty.
    # the second part were the input is not empty
    # here sessions are used to decide on the current page to get the coorect data
    f.write("        if (isset($_REQUEST['Search'])) {\n")
    f.write("            $input = $_GET['searchinput'];\n")
    f.write("            if (empty($input)) {\n")

    # this loop is for tables it will go through all chosen tables to write their data
    # the queries syntax will change depending on the database type.
    for i in range(tableslength):
        f.write("            if ($_SESSION['" + filename + "CurrentPage'] == '" + wantedtables[i] + "') {\n")
        f.write('                echo"<tr>";\n')
        for j in range(columnslength):
        ...
        #this if statement to check if ther are an add request
        ...
        f.write('                     }\n')
        f.write("                }\n")
        # here the select query is decided by the database type.
        # this is the query used for getting the data from the database to show them in the table
        if (dbtype == "PostgreSQL"):
        ...
        f.write('                        echo "<tr>";\n')
        f.write('                        echo "<form>";\n')
        # here the input fields to get the data from the user.
        for j in range(columnslength):
            if (wantedcolumns[j][0] == i):
                ...
                f.write("'>")
                f.write('</td>";\n')
        # here the were we to check for the primary key of the table
        # if there is no primary key the last column of teh table will be chosen as a reference variable for queries
        checker = 0
        g = 0
        for j in range(columnslength):
            if (wantedcolumns[j][0] == i):
                g=j
                if(int(wantedcolumns[j][2]) == 1):
                    f.write("                        $UserSr = $row['" + wantedcolumns[j][1] + "'];\n")
                    checker = 1
        if(checker == 0):
            f.write("                        $UserSr = $row['" + wantedcolumns[g][1] + "'];\n")
        ...
        checker = 0
        g = 0
        # here the delete query is being written
        for j in range(columnslength):
            if (wantedcolumns[j][0] == i):
                g=j
                if(int(wantedcolumns[j][2]) == 1):
        ...
        #here the update function is being written
        ...
        for j in range(columnslength):
            if (wantedcolumns[j][0] == i):
                ...
                if(int(wantedcolumns[j][2]) == 1):
                    ...
                    checker = 1
        if (checker == 0):
            f.write(" WHERE " + wantedcolumns[g][1] + " ='$PassedSr';")
        f.write('";\n')
        f.write('                            $conn->exec($sqlgetStatus);\n')
        f.write('                            $conn = null;\n')
        f.write("                            echo '<meta http-equiv=")
        ...
        f.write("            }\n")
    f.write("            } else {\n")
    # this is the part where we check the input of the search if it is only spaces
    ...
    f.write("                }else {\n")
    # this is the part where the function print the table if there are no search requests
    for i in range(tableslength):
        f.write("            if ($_SESSION['" + filename + "CurrentPage'] == '" + wantedtables[i] + "') {\n")
        f.write('                echo"<tr>";\n')
        for j in range(columnslength):
...
# this function write the code for the footer of the page
def Footer(filename):
    f = open(filename + ".php", "a+")
    f.write("        <footer>\n")
...
# this function combine teh php page parts to write the file
def generate(userID, wantedtables, wantedcolumns, filename, dbtype="sqlite"):
    ...
    try:
        if (sqlite3.connect("Data.sqlite3")):
                ...
                cur = conn.cursor()
                cur.execute('INSERT INTO HISTORY (user,PageName,Design) VALUES('+str(userID)+','+"'"+os.path.basename(filename)+"'"+',2);')
                conn.commit()
                conn.close()
        else:
                print("Failed")
...
# the below commented code is used for running the functions above for testing the functions

#MySQL
# ta = MySQLTables("project")
# cols = MySQLColumns(ta,"project")
# print(cols)
# generate("6",ta,cols,"MySQL_Project3","MySQL")

#Sqlite
# ta = tables()
# cols = columns(ta)
# print(cols)
# generate("6",ta,cols,"sqlite_Data2","sqlite")

# Postgre
# ta = PostgreTable()
# cols = PostgreColumns(ta)
# print(cols)
# generate(6,ta,cols,"PostgreSQL_Project2","PostgreSQL")
