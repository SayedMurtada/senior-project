...
def SearchAndTable(wantedtables, wantedcolumns, filename, dbtype="sqlite"):
    ...
    for i in range(tableslength):
        ...
        if (dbtype == "PostgreSQL"):
            for j in range(columnslength):
                if (wantedcolumns[j][0] == i):
                    if ("CHAR" in wantedcolumns[j][3] or "VARCHAR" in wantedcolumns[j][3]
                            or "TEXT" in wantedcolumns[j][3] or "var" in wantedcolumns[j][3]):
        ...
        f.write("                        if (isset($_REQUEST['delete']) && !isset($_GET['delete'])) {\n")
        f.write("                            try{\n")
        primary=[]
        if (dbtype == "PostgreSQL"):
            for j in range(columnslength):
                if (wantedcolumns[j][2] == 1 and wantedcolumns[j][0] == i):
                    primary.append(wantedcolumns[j])
                    f.write("                            $sqlgetStatus = 'DELETE FROM " + '"' + wantedtables[i]
                            + '" WHERE "' + wantedcolumns[j][1] + '" =')
            if ("CHAR" in primary[0][3] or "VARCHAR" in primary[0][3]
                    or "TEXT" in primary[0][3] or "var" in primary[0][3]):
                f.write("'" + '"' + ". $_GET['delete'] ." + '"' + "';" + '";\n')
            else:
                f.write("'" + ". $_GET['delete'] .'" + ";" + "';\n")
        else:
        ...
        f.write("                                 echo'<script type=" + '"text/javascript">confirm("'
                + "'.$e->getMessage().'" + '");</script>' + "';\n")
        f.write('                             }\n')
        f.write("                        } else if(isset($_REQUEST['savechanges']) && !isset($_GET['savechanges'])){\n")
        f.write("                            try{\n")
        ...
        if (dbtype == "PostgreSQL"):
            ...
            for n in range(columnslength):
                if (wantedcolumns[n][0] == i):
                    ...
                    if ("CHAR" in wantedcolumns[n][3] or "VARCHAR" in wantedcolumns[n][3]
                            or "TEXT" in wantedcolumns[n][3] or "var" in wantedcolumns[n][3]):
                        sargu = sargu + " ='"
                        sargu = sargu + '.' + '"' + "'" + '"'
            ...
            f.write(" SET " + sargu)
            for j in range(columnslength):
                # print(wantedcolumns[j][2])
                # print()
                if (wantedcolumns[j][2] == 1 and wantedcolumns[j][0] == i):
                    if ("CHAR" in wantedcolumns[j][3] or "VARCHAR" in wantedcolumns[j][3]
                            or "TEXT" in wantedcolumns[j][3] or "var" in wantedcolumns[j][3]):
                        f.write(' WHERE "' + wantedcolumns[j][1] + '"' + '='
                                + "'" + '.' + '"' + "'" + '".' + '$PassedSr' + '.' + '"' + "'" + '"' + ';')
    ...
    for i in range(tableslength):
        ...
        f.write("                }\n")
        if (dbtype == "PostgreSQL"):
            for z in range(columnslength):
                if (wantedcolumns[z][0] == i):
                    f.write('                if($_GET["reference"] == "'+wantedcolumns[z][1]+'" ){\n')
                    if("CHAR" in wantedcolumns[z][3] or "VARCHAR" in wantedcolumns[z][3] or "TEXT" in wantedcolumns[z][3]
                            or "var" in wantedcolumns[z][3]):
