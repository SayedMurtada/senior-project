...
# PostgreSQL we need the types of columns to know when should use single or double cotations.
...
# create a function for changing default in select look and feel page.

# add in session user to use it in the set default design
...
def generate(userID,wantedtables, wantedcolumns, filename, dbtype="sqlite"):
    ...
    f.write("   $_SESSION['user']='"+userID+"';\n")
    f.write("}\n")
    if (dbtype != "sqlite"):
        f.write("if (!empty($_SESSION['"+filename+"HostDB']) && !empty($_SESSION['"+filename+"UserDB']) && !empty($_SESSION['"+filename+"DatabaseDB'])) {\n")
    else:
    ...
    f.write("        <link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css' rel='stylesheet'/>\n")
    ...
    f.write("            try{\n")
    f.write('                $conn2 =new PDO("sqlite:".')
    f.write("$_POST['Path']")
    f.write(');\n')
    f.write('                $sql = "SELECT Design FROM History WHERE PageName ='+"'"+filename+"'"+' and user='+"'"+userID+"'"+'";\n')
    f.write('                $getresult = $conn2->query($sql);\n')
    f.write('                $result = $getresult->fetch();\n')
    f.write('                $sql = "SELECT Name FROM Design WHERE Sr= $result";\n')
    f.write('                $getresult = $conn2->query($sql);\n')
    f.write('                $result = $getresult->fetch();\n')
    f.write('                echo "<link rel='+"'stylesheet' href='./Stylesheets/"+'"'+".$result['Design']."
            +'"'+".css'>"+'";\n')
    f.write("            }catch(Exception $e){\n")
    f.write("                echo'<script>alert("+'"could not get Style File"'+")</script>';\n")
    f.write("            }\n")
    f.write("        }else{\n")
    f.write('                echo "<link rel=' + "'stylesheet' href='./Stylesheets/MyStyle.css'>" + '";\n')
    f.write("        }\n")
    f.write("        ?>\n")
    ...
    f.write("            <?php\n")#done
    for i in range(tableslength):
        f.write("            if ($_SESSION['" + filename + "CurrentPage'] == '"+wantedtables[i]+"' && !empty($_SESSION['"+filename+"Connected'])) {\n")
    ...
    f.write("            ?>\n")
    f.write("            <hr>\n")
    f.write("            <button class='navbar-toggler' type='button' data-toggle='collapse' data-target='#navbarSupportedContent' aria-controls='navbarSupportedContent' aria-expanded='false' aria-label='Toggle navigation'>\n")
    ...
    if(dbtype != "sqlite"):
        ...
        f.write('                        echo"<li class='); f.write(" 'nav-item'>");f.write('";\n');

        f.write('                        echo"<h1 id='); f.write(" 'logoh1'>Connected successfully</h1>"); f.write('";\n');
        f.write('                        echo"</li>";\n')
    ...
    if (dbtype != "sqlite"):
        f.write("                        $input = !empty($_POST['Host']) && !empty($_POST['User']) && !empty($_POST['Database']);\n")
    else:
    ...
    for i in range(tableslength):
        ...
        f.write("'"+wantedtables[i]+"' value = "+wantedtables[i]+" class='btn generate1'>"+wantedtables[i]+"</button>")
        f.write('";\n')
    ...
    f.write('                    echo "<input class=')
    f.write("'form-control col-md-8' name='searchinput' type='search' placeholder='Search Activities' style='margin-left: 13%;'")
    f.write('>";\n')
    f.write('                    echo "<button class=')
    ...
    for i in range(tableslength):
        ...
        for j in range(columnslength):
            if (wantedcolumns[j][2] == 1 and wantedcolumns[j][0] == i ):
                f.write('                            $sqlgetStatus = "DELETE FROM '+wantedtables[i]+' WHERE '+wantedcolumns[j][1]+' =')
        f.write("'"+'"'+ ". $_GET['delete'] ."+'"'+"';"+'";\n')
        ...
        for n in range(columnslength):
            if (wantedcolumns[n][0] == i):
                sargu = sargu + wantedcolumns[n][1]+ "='"+ '"'+ ". $_GET['"+wantedcolumns[n][1]+"'] ."+ '"'+ "',"
        sargu = sargu[:-1]
        f.write(" SET "+sargu)
        for j in range(columnslength):
            if (wantedcolumns[j][2] == 1 and wantedcolumns[j][0] == i):
    ...
    for i in range(tableslength):
        ...
        f.write("                     $getStatus = $conn->exec($sqlgetStatus);\n")
        f.write("                     $conn = null;\n")
        f.write("                     echo '<meta http-equiv="+'"refresh" content="0; URL='+filename+'.php"'+"/>';\n")
        f.write('                     }catch(Exception $e){\n')
        # echo '<script type="text/javascript">confirm("'.$e->getMessage().'");</script>';
        f.write("                                 echo'<script type=" + '"text/javascript">confirm("'
                + "'.$e->getMessage().'" + '");</script>' + "';\n")
        ...
        f.write('                            $conn->exec($sqlgetStatus);\n')
        f.write('                            $conn = null;\n')
        f.write("                            echo '<meta http-equiv=");f.write('"refresh" content="0; URL=' + filename + '.php"');
        f.write("/>';\n")
        f.write('                            }catch(Exception $e){\n')
        f.write("                                 $_REQUEST['savechanges']=null;\n")
        # echo '<script type="text/javascript">confirm("'.$e->getMessage().'");</script>';
    ...
    for i in range(tableslength):
        ...
        f.write("                }\n")
        if (dbtype == "PostgreSQL"):
            f.write("                $sqlUsers = 'SELECT " + '"' + argu + ' FROM "' + wantedtables[i] + '"' + ' WHERE "'+"'"+' . $_GET["reference"] .'+"'"+'" =')
            f.write('"')
        ...
        f.write('                            $conn = null;\n')
        f.write("                            echo '<meta http-equiv=")
        f.write(' "refresh" content="0; URL=' + filename + '.php"');f.write(" />';\n");
        f.write('                            }catch(Exception $e){\n')
        f.write("                                 $_REQUEST['savechanges']=null;\n")
        # echo '<script type="text/javascript">confirm("'.$e->getMessage().'");</script>';
        f.write("                                 echo'<script type=" + '"text/javascript">confirm("'
    ...
    for i in range(tableslength):
        ...
        for j in range(columnslength):
            if(wantedcolumns[j][0] == i):
                f.write('                echo"<th id='); f.write(" '"+wantedcolumns[j][1]+"'>"+wantedcolumns[j][1]+"</th>"); f.write('";\n');
        f.write('                echo"<th id='); f.write("actions'>Actions</th>"); f.write('";\n');
        f.write('                echo"</tr>";\n')
        ...
        for j in range(columnslength):
            if (wantedcolumns[j][0] == i):
                f.write('                        echo "<td><input type=')
                f.write("'text' name='"+wantedcolumns[j][1]+"' value='"+'".$row["'+wantedcolumns[j][1]+'"] ."'+"'>"+'</td>";\n')
        ...
        for j in range(columnslength):
            if (wantedcolumns[j][2] == 1 and wantedcolumns[j][0] == i):
                f.write('                            $sqlgetStatus = "DELETE FROM '+wantedtables[i]+' WHERE '+wantedcolumns[j][1]+' =')
        f.write(" '"); f.write(' "'); f.write(". $_GET['delete'] ."); f.write(' "');f.write(" ';"); f.write('";\n');
        f.write('                            $conn->exec($sqlgetStatus);\n')
        f.write('                            $conn = null;\n')
        f.write("                            echo '<meta http-equiv=");f.write(' "refresh" content="0; URL='+filename+'.php"'+"/>';\n");
        f.write('                            }catch(Exception $e){\n')
    ...
    try:
        if (sqlite3.connect("Data.sqlite3")):
                ...
                cur = conn.cursor()
                cur.execute('INSERT INTO HISTORY (user,PageName,Design) VALUES('+userID+','+"'"+'sqlite_Project'+"'"+',1);')
                conn.commit()
                conn.close()
        else:
                print("Failed")
...
# print(cols)
# generate(ta,cols,"MySQL_Project2","MySQL")
#Sqlite
# ta = tables()
# cols = columns(ta)
# print(cols)
# generate("2",ta,cols,"sqlite_Project","sqlite")

# Postgre
ta = PostgreTable()
cols = PostgreColumns(ta)
print(cols)
generate(ta,cols,"PostgreSQL_Project","PostgreSQL")
